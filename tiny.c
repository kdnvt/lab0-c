...
void format_size(char *buf, struct stat *stat)
{
    if (S_ISDIR(stat->st_mode)) {
        ...
        if (size < 1024) {
            snprintf(buf, MAXLINE, "%lu", size);
        } else if (size < 1024 * 1024) {
            snprintf(buf, MAXLINE, "%.1fK", (double) size / 1024);
        } else if (size < 1024 * 1024 * 1024) {
            snprintf(buf, MAXLINE, "%.1fM", (double) size / 1024 / 1024);
        } else {
            snprintf(buf, MAXLINE, "%.1fG", (double) size / 1024 / 1024 / 1024);
        }
    }
}
...
void handle_directory_request(int out_fd, int dir_fd, char *filename)
{
    ...
    while ((dp = readdir(d)) != NULL) {
        ...
        if (S_ISREG(statbuf.st_mode) || S_ISDIR(statbuf.st_mode)) {
            char *tmpd = S_ISDIR(statbuf.st_mode) ? "/" : "";
            snprintf(buf, MAXLINE,
                     "<tr><td><a "
                     "href=\"%s%s\">%s%s</a></td><td>%s</td><td>%s</td></tr>\n",
                     dp->d_name, tmpd, dp->d_name, tmpd, m_time, size);
            writen(out_fd, buf, strlen(buf));
        }
        close(ffd);
    }
...
void parse_request(int fd, http_request *req)
{
    ...
    sscanf(buf, "%256s %256s", method, uri); /* version is not cared */
    /* read all */
    while (buf[0] != '\n' && buf[1] != '\n') { /* \n || \r\n */
        rio_readlineb(&rio, buf, MAXLINE);
...
void serve_static(int out_fd, int in_fd, http_request *req, size_t total_size)
{
    ...
    if (req->offset > 0) {
        ...
        snprintf(buf + strlen(buf), 256, "Content-Range: bytes %lu-%zu/%zu\r\n",
                 req->offset, req->end, total_size);
    } else {
        snprintf(buf, 256, "HTTP/1.1 200 OK\r\nAccept-Ranges: bytes\r\n");
    }
    snprintf(buf + strlen(buf), 256, "Cache-Control: no-cache\r\n");
    // snprintf(buf + strlen(buf),MAXLINE, "Cache-Control: public,
